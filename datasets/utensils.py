...
class UtensilsDataset(Dataset):
    """
    Dataset for Spoon / knife classification.

    Root directory layout assumed (relative to your --data path):
        utensils/
            train/
                spoon/
                knife/
    ...
    d_type should be 'train' or 'test'.
    """

    labels = ['spoon', 'knife']
    label_to_id_map = {k: v for v, k in enumerate(labels)}
    label_to_folder_map = {'spoon': 'spoon', 'knife': 'knife'}
    ...
    def __init__(self, root_dir, d_type, transform=None,
        ...
        print("DEBUG: data_list sample entries:", self.data_list[:5])
    ...
    def __print_manual_instructions(self):
        ...
        print("  <data_root>/utensils/train/spoon")
        print("  <data_root>/utensils/train/knife")
        print("  <data_root>/utensils/test/spoon")
        print("  <data_root>/utensils/test/knife")
    ...
    def __getitem__(self, index):
        ...
        if image is None:
            print(f"WARNING: cv2.imread failed for {image_path}, skipping sample")
        ...
        try:
            ...
            print(f"WARNING: cvtColor failed for {image_path}: {e}")
            next_idx = (index + 1) % len(self)
            return self.__getitem__(next_idx)
...
def get_utensils_dataset(data, load_train, load_test):
    (data_dir, args) = data
    ...
    test_ds = None
    if load_train:
        train_ds = UtensilsDataset(root_dir=os.path.join(data_dir, 'utensils'),
                                   d_type='train',
                                   transform=transform,
                                   augment_data=True)
    if load_test:
        test_ds = UtensilsDataset(root_dir=os.path.join(data_dir, 'utensils'),
                                  d_type='test',
                                  transform=transform,
                                  augment_data=False)
