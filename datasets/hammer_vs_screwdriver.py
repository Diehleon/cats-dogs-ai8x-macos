...
class HammerVsScrewdriver(Dataset):
    """
    Hammer vs Screwdriver Dataset.

    Root directory layout assumed (relative to your --data path):
        hammer_vs_screwdriver/
            train/
                hammer/
                screwdriver/
            test/
    ...
    label_to_folder_map = {'hammer': 'hammer', 'screwdriver': 'screwdriver'}

    def __init__(self, root_dir, d_type, transform=None,
                 resize_size=(128, 128), augment_data=False):
        self.root_dir = root_dir
        # expecting root_dir like ".../data/hammer_vs_screwdriver"
    ...
    def __print_manual_instructions(self):
        ...
        print("  <data_root>/hammer_vs_screwdriver/train/hammer")
        print("  <data_root>/hammer_vs_screwdriver/train/screwdriver")
        print("  <data_root>/hammer_vs_screwdriver/test/hammer")
        print("  <data_root>/hammer_vs_screwdriver/test/screwdriver")
...
def get_hammer_vs_screwdriver_dataset(data, load_train, load_test):
    (data_dir, args) = data
    ...
    test_ds = None
    if load_train:
        train_ds = HammerVsScrewdriver(root_dir=data_dir, d_type='train',
                                       transform=transform, augment_data=True)
    if load_test:
        test_ds = HammerVsScrewdriver(root_dir=data_dir, d_type='test',
                                      transform=transform, augment_data=False)
    return train_ds, test_ds

datasets = [
    {
        'name': 'hammer_vs_screwdriver',
        'input': (3, 128, 128),
        'output': ('hammer', 'screwdriver'),
        'loader': get_hammer_vs_screwdriver_dataset,
    },
]
